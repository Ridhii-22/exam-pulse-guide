...
import { subjectProgress as fallbackSubjects, dailyGoals, activity as fallbackActivity } from "@/lib/mock-data";
...
function HomePage() {
  ...
  // build last 7 days activity for the mini bar chart
  ...
  const hasActivity = recent7.some((r) => r.value > 0);
  const weekly = hasActivity ? recent7 : fallbackActivity;
  const maxV = Math.max(1, ...weekly.map((w) => w.value));

  const hasSubjects = (data?.subjectAccuracy?.length ?? 0) > 0;
  const subjectRows = hasSubjects
    ? data!.subjectAccuracy.map((s) => ({ subject: s.subject, solved: s.solved, total: Math.max(s.solved, 100), accuracy: s.accuracy }))
    : fallbackSubjects.map((s) => ({ ...s, solved: 0, total: s.total }));
    ...
    <AppShell>
      <Container>
        <PageHeader
          ...
          subtitle={streak > 0 ? `You're on a ${streak}-day streak. Let's keep the momentum.` : "Start a session today to begin your streak."}
          action={
            <Link to="/tests">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-8">
          ...
          <Stat label="Accuracy" value={`${accuracy}%`} hint="All-time" icon={<Target className="size-5" />} />
          <Stat label="Study time" value={`${studyHours}h`} hint="All-time" icon={<Clock className="size-5" />} />
          <Stat label="XP" value={xp.toLocaleString()} hint={`Level ${level}`} icon={<TrendingUp className="size-5" />} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2 space-y-5">
            <Card className="relative overflow-hidden">
              ...
              <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Resume a lecture or jump into a quick test.</p>
                </div>
                <Link to="/lectures">
                  <Button>Browse lectures <ChevronRight className="size-4" /></Button>
                </Link>
              </div>
            </Card>
                ...
                {subjectRows.map((s) => (
                  <div key={s.subject}>
                    <div className="flex justify-between text-sm mb-1.5">
                      ...
                      <span className="text-muted-foreground">{s.solved.toLocaleString()} solved · {s.accuracy}%</span>
                    </div>
            ...
            <Card>
              ...
              {recentTests.length === 0 ? (
                <p className="text-sm text-muted-foreground">No tests yet. Take a mock to see results here.</p>
              ) : (
          ...
          <div className="space-y-5">
            ...
            <Card>
              ...
              {weak.length === 0 ? (
                <p className="text-sm text-muted-foreground">Take a few tests and we'll highlight chapters to revise.</p>
              ) : (
              ...
              <div className="flex items-end gap-2 h-24">
                {weekly.map((a, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1.5">
                    <div className="w-full rounded-md bg-primary/70 hover:bg-primary transition" style={{ height: `${Math.max(4, (a.value / maxV) * 100)}%` }} />
